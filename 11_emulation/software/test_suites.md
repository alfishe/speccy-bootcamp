...
2. **Memory and timing behaviour** — contended memory timing, ULA video timing, interrupt timing
3. **Peripherals** — AY-3-8912 sound chip, Kempston joystick, Interface 1 microdrives, etc.

A modern emulator that passes all the major test suites will run essentially all original Spectrum software correctly. Conversely, an emulator that fails tests will produce visible glitches on specific software — particularly demoscene productions, which often rely on precise hardware behaviour.
...
- **Undocumented instructions** — the Z80 has dozens of undocumented opcodes (e.g., `SLI`, `SLL`, the half-carry flag behaviour on `BIT n, (HL)`). Real Spectrum software uses them, particularly games from Eastern Europe.
...
- **Interrupt timing** — the Z80's interrupt response time depends on what the CPU is doing when the interrupt is asserted; the ULA's INT signal must be modelled cycle-accurately.
...
2. **Targeted validation** — when adding support for a new piece of hardware (e.g., a new Russian clone), the author runs the relevant test suites to confirm the new hardware is correctly modelled.

Real hardware is also tested with these suites — the same test program that fails on emulator X may also fail (or behave subtlyly differently) on a real Pentagon vs a real 48K Spectrum. The test suites thus serve a dual purpose: validating emulators *and* documenting hardware behaviour.
...
- **ZEXDOC** tests the documented behaviour of every instruction — every register combination, every flag, every addressing mode
...
- Wrong flag behaviour (the Z80 has subtle flag-setting rules that are easy to get wrong)
- Wrong undocumented instruction behaviour
- Wrong register pair behaviour (e.g., `LD (HL), r` not properly handling the `(HL)` addressing mode)
...
- **Contended memory timing tests** — verify that the ULA's contention pattern is correctly modelled
...
- **Audio tests** — verify AY-3-8912 register behaviour and timing
...
Several test programs focus specifically on **cycle-exact timing behaviour**:

#### Sensible Software Tests

- **"Sensible" tests** — a series of small programs written by **Andrew Owen** that demonstrate specific timing-sensitive effects (ULA Plus palette switching, multicolour effects, raster interrupts). Emulators that get the timing wrong produce visible glitches on these tests.
- **"Float Spell"** — a popular multicolour test/demo that requires cycle-accurate ULA timing
- **"Sixteen colours" / "128-colour" demos** — push the limits of the Spectrum's colour capabilities, requiring cycle-accurate contention modelling
...
The AY-3-8912 sound chip has its own subtle behaviours that test programs verify:

- **Register write order** — writing to the AY involves a 2-step process (register select, then data); the timing matters
- **Envelope generator behaviour** — the AY's envelope generator has specific reset conditions
...
The Kempston joystick interface is read at I/O address `0x1F`. Test programs verify:

- The joystick bits are correctly mapped (bit 0 = right, bit 1 = left, etc.)
- Reading from `0x1F` doesn't affect other hardware
...
4. **Test the specific behaviour you care about** — if you're developing for a 128K +2A, run the tests on a +2A emulated configuration, not the default 48K
...
- **They don't cover everything** — some hardware behaviours (CRT screen phosphor decay, RF interference, power supply sag) are not testable in software.

For these reasons, even emulators that pass all known test suites should be considered "asymptotically correct" rather than perfect. Real hardware remains the ultimate reference for any specific software behaviour.
...
A: No. ZEXALL only tests Z80 instruction correctness — it says nothing about timing, interrupts, peripherals, or ULA behaviour. An emulator that passes ZEXALL but has timing bugs will still fail on most demoscene productions.
...
A: Yes — and the community welcomes new test programs. If you discover a hardware behaviour that no existing test covers (e.g., a subtle interaction between two peripherals), write a small test program and contribute it. The FUSE test suite accepts community submissions via the project's source repository.
...
**Q: What's the hardest behaviour to test?**
