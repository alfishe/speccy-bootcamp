...
The 256-byte intro is the **most constrained** recognised category. On the Z80, 256 bytes is roughly:

- 50–80 instructions of "naive" code, or
- 100–150 instructions of carefully size-optimised code, or
...
1K rules are largely standardised across parties:
...
- **Raster bars** (border-only, port `#FE` writes synchronised to INT).
...
The Pentagon ROM is **derived from the 48K ROM** and most routines work at the same address. However, the Pentagon has no `BEEPER` contention behaviour, so timing-dependent calls to `BEEPER` produce slightly different pitch.
...
5. **Measure** the total size; if still over budget, go back to §5–§7 and optimise the source further.
...
- **Border-only raster bars** with up to 8 simultaneous colours.
...
- **Logan & O'Hara, "The Spectrum ROM Disassembly"** (1983) — the canonical reference for ROM routine addresses.
- **Pouet.net** — searchable archive of size-coded Spectrum productions.
