...
| File | Topic |
|---|---|
| `interrupt_overview.md` | ZX Spectrum interrupt system: how INT is generated by the ULA, per-model timing (48K vs 128K vs Pentagon), INT line behavior |
| `im1_programming.md` | IM1 (default): ROM ISR at #0038, hooking via system variables, limitations |
| `im2_programming.md` | IM2 deep dive: vector table setup, 257-byte alignment, ISR skeleton, timing constraints |
| `isr_patterns.md` | ISR design patterns: minimal ISR, deferred processing, multi-source dispatch, re-entrant ISR |
| `interrupt_timing.md` | Interrupt timing per model: when INT fires (scanline 248), T-state precision, jitter, per-model differences |
| `race_the_beam.md` | Race-the-beam programming: synchronizing code to raster position for multicolor, border effects, scanline tricks |
| `nmi.md` | NMI handling: Multiface NMI, NMI button, what's safe in NMI context |
