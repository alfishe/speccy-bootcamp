...
│   ├── 09_gamedev/                  # ✅ F13 COMPLETE
│   │   ├── README.md
│   │   ├── game_loop.md
│   │   ├── entities_collision_ai.md
│   │   ├── level_data_and_worlds.md
│   │   ├── input_sound_integration.md
...
| File | Topic |
|---|---|
| `README.md` | Section index — 3-article F14 series, reading order for 3 reader profiles (first-time / developers / demoscene), cross-reference list, descope note for the merged timeline |
| `history.md` | **Narrative synthesis of the ZX Spectrum's 40+ year story**: five eras (Pre-Spectrum 1972–1981, Sinclair 1982–1986, Amstrad 1986–1992, Post-Soviet clone 1989–2000s, Modern revival 2010s–present), verified dates from canonical Sinclair Wiki source, embedded Mermaid `timeline` diagram 1976–2022, cultural/economic context, 5 pitfalls |
| `hardware_models.md` | **Navigational hub for hardware models across three tracks**: Original/Soviet Clones/New Gen taxonomy, three per-track model tables (7 Original + 10 Soviet clones + 8 New Gen), three cross-track comparison matrices (frame timing, memory architecture, sound capability), "How to Choose a Model" decision matrix with 8 use cases |
| `glossary.md` | **Platform-specific terminology reference**: 100+ terms organized by category (Hardware, Display, Memory, Storage, Software/System, Cultural/Demoscene, Track-Specific), each entry has 2–4 sentence definition with "See X" cross-reference, abbreviations listed both short and full form |
...
### 02 — Hardware (subfoldered by stream) ✅ COMPLETE

#### 02_hardware/original/ — Sinclair/Amstrad ✅ COMPLETE (8 articles + README; 4 descoped)
...
#### 02_hardware/clones/ — Soviet Clone Ecosystem ✅ COMPLETE (11 articles + README; `ula_replacements.md` descoped — covered briefly in [README.md](02_hardware/clones/README.md) and per-clone articles)
...
#### 02_hardware/newgen/ — New Generation ✅ COMPLETE (7 articles + README; consolidated from 15 planned)
...
| `rom_versions.md` | ROM version catalog: 48K Issues 1-6 CRC32, 128K, +2A/+3, clone ROMs, modern replacements | ✅ |
...
| `interlace_and_flicker.md` | Spectrum's non-interlaced output, 50 Hz perception threshold, attribute flicker, GigaScreen flicker math, CRT vs LCD behavior | ✅ |
...
#### 05_development/09_gamedev/ — Game Development ✅ F13 COMPLETE

| File | Topic |
|---|---|
| `README.md` | Index — 5-article consolidated F13 series, reading order, cross-reference table |
| `game_loop.md` | **Frame budget and sync architectures**: HALT-driven (48K) vs ISR-driven (128K Ocean/Rare) loops, frame-synchronized update at ~50 Hz, T-state budget breakdown (69,888 / 70,908 / 71,680 for 48K / 128K / Pentagon), flat vs hierarchical state machines, 48K flat vs 128K banked memory models, 4 ROM/RAM layouts (All-RAM tape / ROM cartridge / Banked 128K / Multiload), load screens and streaming |
| `entities_collision_ai.md` | **Entity subsystem and AI**: object pool pattern (8 bytes/entity, 16–32 slots, type=0 sentinel), update ordering, dual-phase erase/draw rendering, 4 collision algorithms (AABB ~80T/pair, grid broad-phase, pixel-precise 200–500T, attribute-based ~30T), tile collision (axis-separated resolution, gravity, one-way platforms, conveyor belts), 5 AI patterns (FSM with state byte dispatch, fixed-path waypoints with Manic Miner 7-byte guardian format, steering, BFS pathfinding), sprite integration interface |
| `level_data_and_worlds.md` | **World model architectures**: 4 canonical world models comparison (single-screen / multi-room persistent / scrolling / isometric), full Matthew Smith 1024-byte room format byte-offset table, persistent state implementation pattern (immutable template + mutable state block), 1-direction vs bidirectional scrolling with camera update code, tile-map render pipeline, compression strategies (RLE, column-major+LZ, ZX0 per-stage packing), room transitions |
| `input_sound_integration.md` | **Input and audio integration layer**: input normalization across devices (Kempston active-high, keyboard/Sinclair active-low, polarity invert before merge), edge detection pattern `(current) AND NOT (previous)`, menu repeat timers, redefine-keys UI pattern, 4 places music can live (foreground / ISR / separate ISR / dedicated hardware), 128K bank-switched music ISR pattern (save/restore #7FFD), SFX channel-stealing pattern with priority scheme, memory budgets |
| `game_case_studies.md` | **Engine case studies — architect's perspective**: Manic Miner (Matthew Smith engine, 9 KB engine), Jet Set Willy (persistent rooms, 60 rooms, multi-waypoint guardians), Knight Lore (Filmation isometric engine, pre-rendered 4-view sprites, painter's algorithm z-sort), Alien 8 (stacking physics, batched sort), Head Over Heels (two-character gameplay, ~300 rooms, 128K AY), Elite (table-driven quarter-square multiply ~100T, 1.15 fixed-point, back-face culling), 6 cross-engine patterns |
...
| `ay_music_formats.md` | **Master catalog**: every AY/YM music file format (`.PT3`, `.PSG`, `.YM`, `.AY`, `.AKG`, etc.) — modules, dumps, containers, modern embedded | ✅ Done |
...
| 05_dev/09_gamedev | **6/6** | ✅ **F13 COMPLETE** — consolidated from 9 planned into **5 comprehensive articles + README** (~2,461 lines total): game_loop.md (502), entities_collision_ai.md (729), level_data_and_worlds.md (413), input_sound_integration.md (435), game_case_studies.md (382). User approved "5-article consolidation" via AskUserQuestion. Cross-verified against 06_graphics (sprites/scrolling/3d), 06_sound (ay_player_routines), 09_toolchain (asset_tools), 03_io (joystick), 08_reverse_engineering (methodology) to avoid duplication. Chunk discipline per memory `4cf58ea7`: 100-250 line Write+SearchReplace calls throughout. |
| 00 Overview | **3/3** | ✅ **F14 COMPLETE** — consolidated from 4 planned into **3 comprehensive articles + README** (~885 lines total): history.md (364 — narrative synthesis with embedded Mermaid timeline 1976–2022, replaces standalone timeline.md), hardware_models.md (203 — navigational hub with three-track taxonomy and 3 cross-track comparison matrices + How to Choose decision matrix), glossary.md (318 — 100+ terms across 7 categories with cross-references). User approved "Consolidate to 3 articles" via AskUserQuestion. Cross-verified against existing per-model History sections, demoscene_history.md, video_frame_comparison.md, and canonical 10_references articles to ensure the overview articles link to rather than duplicate deeper content. Chunk discipline per memory `4cf58ea7`: all articles written in 100-250 line Write+SearchReplace chunks. |
...
- **Tier F13 — Game Development series** ✅ **COMPLETE** (Jul 2026): Consolidated from 9 planned articles into **5 comprehensive articles + README** (~2,461 lines total): `game_loop.md` (502 — universal 8-bit loop, 3 sync patterns HALT-driven / ISR-driven with frame_flag synchronization / cycle-counted, frame-synchronized budgeting 69,888 T-states, flat vs hierarchical state machines, 48K flat vs 128K banked memory models, 4 ROM/RAM layouts All-RAM/ROM cartridge/Banked 128K/Multiload, load screens and streaming, debugging symptoms, 11 cross-refs, 6 pitfalls), `entities_collision_ai.md` (729 — object pool pattern 8 bytes/entity with type=0 sentinel, update ordering, dual-phase erase/draw rendering, 4 collision algorithms AABB ~80T/pair + grid broad-phase + pixel-precise 200–500T + attribute-based ~30T, axis-separated tile collision resolution, gravity and one-way platforms and conveyor belts, 5 AI patterns FSM + waypoints + steering + Manic Miner 7-byte guardian format + BFS pathfinding, sprite integration interface, 8 cross-refs, 6 pitfalls), `level_data_and_worlds.md` (413 — 4 canonical world models comparison table, full Matthew Smith 1024-byte room format byte-offset table, screen-layout-as-collision-data trick, persistent state pattern immutable template + mutable state block, 1-direction vs bidirectional scrolling with camera update code, tile-map render pipeline, compression strategies RLE + column-major+LZ + tile-attr separation + per-stage ZX0, room transitions, 8 cross-refs, 6 pitfalls), `input_sound_integration.md` (435 — input normalization across devices Kempston active-high vs keyboard/Sinclair active-low with polarity invert before merge, edge detection pattern `(current) AND NOT (previous)`, menu repeat timers, redefine-keys UI pattern with Western vs post-Soviet conventions, 4 places music can live foreground / ISR / separate ISR / dedicated hardware, 128K bank-switched music ISR pattern save/restore #7FFD, SFX channel-stealing with priority scheme, memory budgets, 9 cross-refs, 7 pitfalls), `game_case_studies.md` (382 — architect's perspective on 6 canonical engines: Manic Miner Matthew Smith engine 9 KB, Jet Set Willy persistent rooms 60 rooms multi-waypoint guardians, Knight Lore Filmation isometric pre-rendered 4-view sprites painter's algorithm z-sort ~3000T, Alien 8 stacking physics batched sort, Head Over Heels two-character gameplay ~300 rooms 128K AY, Elite table-driven quarter-square multiply ~100T 1.15 fixed-point back-face culling, 6 cross-engine patterns, 7 cross-refs, 6 pitfalls). Differentiated from `08_reverse_engineering/methodology.md` §5.3 by framing case studies as architect's perspective (design decisions each engine embodies) rather than RE perspective (how to identify engines from disassembly). Cross-verified against 10 existing articles (sprite primitives, scrolling, 3D math, AY player internals, asset tools, input hardware, frame timing, interrupts, RE fingerprints, demo frameworks) to avoid duplication. Triggered by user instruction "what next?" → selected "F13 Game Development" → user approved "5-article consolidation" via AskUserQuestion. Chunk discipline per memory `4cf58ea7`: all articles written in 100-250 line Write+SearchReplace chunks.
- **Tier F14 — Overview series** ✅ **COMPLETE** (Jul 2026): Consolidated from 4 planned articles into **3 comprehensive articles + README** (~885 lines total): `history.md` (364 — narrative synthesis covering five eras Pre-Spectrum 1972–1981 / Sinclair 1982–1986 / Amstrad 1986–1992 / Post-Soviet clone 1989–2000s / Modern revival 2010s–present, with verified dates from canonical Sinclair Wiki Timeline source, embedded Mermaid `timeline` diagram 1976–2022 spanning Pre-Spectrum era through modern demoscene parties, cultural and economic context for each transition, 5 pitfalls including sales figures contested and Amstrad acquisition date accuracy and Pentagon as reimplementation not copy), `hardware_models.md` (203 — navigational hub with three-track taxonomy Original/Soviet Clones/New Gen, three per-track model tables covering 25 models total (7 Original + 10 Soviet clones + 8 New Gen), three cross-track comparison matrices frame timing × 10 models × 7 columns + memory architecture + sound capability, "How to Choose a Model" decision matrix with 8 use cases mapped to specific model recommendations, every model row links to its per-model deep-dive article in 02_hardware/), `glossary.md` (318 — 100+ terms organized across 7 categories Hardware / Display / Memory / Storage / Software & System / Cultural & Demoscene / Track-Specific, each entry has 2–4 sentence definition with "See X" cross-reference, abbreviations listed both short form (ULA, AY, PSG, ISR, NMI) and full form (Uncommitted Logic Array), 5 pitfalls including Spectrum vs Speccy naming and Pentagon disambiguation and three meanings of "ROM" and Next as separate hardware family and AY vs PSG terminology). The standalone `timeline.md` was descoped by merging its content into `history.md` §6 as an embedded Mermaid timeline diagram — the user approved this "Consolidate to 3 articles" option via AskUserQuestion because a separate chronological-visualization article duplicated content already covered by the narrative and by per-model History subsections. Cross-verified against existing per-model History sections in 02_hardware/, demoscene_history.md, video_frame_comparison.md, and canonical 10_references articles (io_port_map, memory_maps, color_palette, rom_routines) to ensure the overview articles link to rather than duplicate deeper content. Triggered by user instruction "what next?" → user first selected "F15: 02 Hardware Original completion" but canceled when Phase 1 revealed all 8 in-scope articles already existed (F1 batch had completed the section, header was stale) → re-presented options → user selected "F14: 00 Overview (4 articles)" → 5-phase research (scan PLAN.md / cross-reference scan / external research via WebSearch + WebFetch Sinclair Wiki Timeline / 4-article delimitation proposal / 3-article consolidation approval). Chunk discipline per memory `4cf58ea7`: all articles written in 100-250 line Write+SearchReplace chunks. **Closes out the last empty section of the ZX Spectrum knowledge base.**
