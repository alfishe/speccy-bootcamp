...
### Port Map (16-bit Address Decoding)

| Port | Address (A15–A0) | Decoding (A15–A0) | R/W | Purpose |
|---|---|---|---|---|
| `#00FF` (255) | `0000000011111111` | `xxxxxxx011111111` | W | SAA1099 register address select |
| `#01FF` (511) | `0000000111111111` | `xxxxxxx111111111` | W | SAA1099 register data write |
| `#04FF` (1279) | `0000010011111111` | `xxxxxxx011111111` | W | Register address (Light/Middle/Extreme) |
| `#05FF` (1535) | `0000010111111111` | `xxxxxxx111111111` | W | Register data (Light/Middle/Extreme) |
| `#FFFD` (65533) | `1111111111111101` | `11xxxxxx11111101` | W | Clock gating via bit 3 |

**Decoding notes:**
- The `x` bits are don't-care — any value matches. This creates **128 mirror addresses** for each SAA1099 port.
- Revisions 00–03 use `#00FF` / `#01FF`, gated by `DOS/` and `IODOS/` signals to avoid Beta 128 disk controller conflicts.
- Revisions Light/Middle/Extreme add `#04FF` / `#05FF` as cleaner ports without TR-DOS gating dependencies.
- Bit 8 (A8) distinguishes address select (0) from data write (1).
...
```z80
; -------------------------------------------------------
; Full SAA1099 Initialization Sequence.
; Call at program start after SAA_SOFT_RESET.
; Initializes all channels to silence with known state.
; Destroys: A, B, C, D, E, HL
; -------------------------------------------------------
SAA_FULL_INIT:
    ; First, do the clock-gate soft reset
    CALL SAA_SOFT_RESET

    ; Clear all frequency registers (#00-#05)
    LD   HL,#0000        ; H=value (0), L=start reg
    LD   B,6             ; 6 frequency regs
.freq_loop:
    LD   D,L
    LD   E,H
    CALL SAA_WRITE_REG
...
.oct_loop:
    ...
    ; Clear all volume registers (#10-#13)
    LD   L,#10
    LD   B,4
.vol_loop:
    LD   D,L
    ...
    ; Disable envelopes and noise (#16, #1C)
    LD   D,#16
    LD   E,0
    CALL SAA_WRITE_REG
    LD   D,#1C
...
The SAA1099 exposes **32 internal registers** (0..31), accessed via the two-port pattern: write the register address to port `#00FF` (or `#04FF`), then write the data to port `#01FF` (or `#05FF`).

#### Frequency Registers (`#00`–`#05`)

| Register | Channel | Bits 7–0 |
|---|---|---|
| `#00` | 0 | 8-bit frequency divider (0–255) |
| `#01` | 1 | 8-bit frequency divider |
| `#02` | 2 | 8-bit frequency divider |
| `#03` | 3 | 8-bit frequency divider |
| `#04` | 4 | 8-bit frequency divider |
| `#05` | 5 | 8-bit frequency divider |

#### Octave Registers (`#08`–`#0A`)

Two channels are packed per register (4 bits each):

| Register | Bits 7–4 | Bits 3–0 |
|---|---|---|
| `#08` | Channel 1 octave (0–7) | Channel 0 octave (0–7) |
| `#09` | Channel 3 octave | Channel 2 octave |
| `#0A` | Channel 5 octave | Channel 4 octave |

#### Mixer / Enable Registers (`#14`–`#15`)

| Register | Bit 5 | Bit 4 | Bit 3 | Bit 2 | Bit 1 | Bit 0 |
|---|---|---|---|---|---|---|
| `#14` | Ch5 noise | Ch4 noise | Ch3 noise | Ch2 noise | Ch1 noise | Ch0 noise |
| `#15` | Ch5 tone | Ch4 tone | Ch3 tone | Ch2 tone | Ch1 tone | Ch0 tone |

#### Volume Registers (`#00`–`#05` range, `#10`–`#15` range)

The SAA1099 has **separate left and right volume** for each channel — this is the stereo panning feature:

| Register | Channel | Bits 7–4 | Bits 3–0 |
|---|---|---|---|
| `#10` | 0/1 | Ch1 left vol (0–15) | Ch0 left vol (0–15) |
| `#11` | 2/3 | Ch3 left vol | Ch2 left vol |
| `#12` | 4/5 | Ch5 left vol | Ch4 left vol |
| `#13` | 0/1 | Ch1 right vol | Ch0 right vol |
| ... | ... | ... | ... |

> [!NOTE]
> Some documentation shows volume registers at `#18`–`#1D` (left) and `#20`–`#25` (right) with one channel per register. The actual layout depends on chip revision. The ZXM-SoundCard documentation uses the packed format above.

#### Noise Control Registers (`#16`)

| Bit | Function |
|---|---|
| 7–6 | Reserved |
| 5–4 | Noise generator 1 frequency: `00`=/2, `01`=/4, `10`=/16, `11`=/256 |
| 3–2 | Noise generator 0 frequency: same encoding |
| 1 | Noise gen 1 enable |
| 0 | Noise gen 0 enable |

#### Envelope Registers (`#18`–`#1C`)

| Register | Function | Bits |
|---|---|---|
| `#18` | Envelope gen 0 frequency | 8-bit |
| `#19` | Envelope gen 1 frequency | 8-bit |
| `#1A` | (Reserved) | — |
| `#1B` | (Reserved) | — |
| `#1C` | Envelope control | See below |

Envelope control (`#1C`) bit layout:

| Bit | Function |
|---|---|
| 7 | Envelope gen 1 external clock |
| 6 | Envelope gen 0 external clock |
| 5 | Envelope gen 1 enable |
| 4 | Envelope gen 0 enable |
| 3 | Envelope gen 1 invert right |
| 2 | Envelope gen 0 invert right |
| 1 | Envelope gen 1 shape (0=sawtooth, 1=triangle) |
| 0 | Envelope gen 0 shape |

#### Sound Enable Register (`#1C`)

| Bit | Function |
|---|---|
| 0 | Sound output enable (1=on, 0=off) |
| 1 | All channels sync reset |

### Frequency Calculation

The output frequency for a given channel depends on the 8-bit frequency register and the 4-bit octave register:

```
f_out = f_clock / (2 × 256 × (256 - freq)) × 2^octave
```

For an 8 MHz master clock (typical ZXM-SoundCard):

| Note | Freq (Hz) | Octave | Freq Reg |
|---|---|---|---|
| C-2 | 65.4 | 0 | 0 |
| C-3 | 130.8 | 1 | 0 |
| C-4 | 261.6 | 2 | 0 |
...
> [!WARNING]
> SAM Coupé uses a 7.09 MHz clock. Frequency tables designed for SAM Coupé will produce notes ~13% sharp on an 8 MHz ZXM-SoundCard. Recalculate or transpose as needed.
